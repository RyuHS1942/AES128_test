...
void expandKey(BYTE* key, BYTE* roundKey) {
	...
	for (int k = 0; k < ROUNDKEY_SIZE / KEY_SIZE; k++) {
			...
			for (int i = 0; i < KEY_SIZE / 4; i++) {
				if (k == 0) {//첫번째 키
					if (j == 3) {//rot
						...
						temp[i] = sbox[(BLOCK_SIZE * (temp[i] / BLOCK_SIZE)) + (temp[i] % BLOCK_SIZE)];//sub
...
BYTE* subBytes(BYTE* block, int mode) {
	...
	case ENC:
		...
		for (int i = 0; i < BLOCK_SIZE; i++) {
			block[i] = sbox[(BLOCK_SIZE * (block[i] / BLOCK_SIZE) + (block[i] % BLOCK_SIZE))];
		}
		break;
	...
	case DEC:
		...
		/* 추가 구현 */
		for (int i = 0; i < BLOCK_SIZE; i++) {
			block[i] = rsbox[(BLOCK_SIZE * (block[i] / BLOCK_SIZE) + (block[i] % BLOCK_SIZE))];
		}
		break;
...
BYTE* mixColumns(BYTE* block, int mode) {
		...
		for (int i = 0; i < BLOCK_SIZE / 4; i++) {
			for (int j = 0; j < BLOCK_SIZE / 4; j++) {
				for (int k = 0; k < BLOCK_SIZE / 4; k++) {
					temp[i + 4 * j] ^= procession[4 * i + k] * block[4 * j + k];
				}
				block[i + 4 * j] = temp[i + 4 * j];
			}
		}
	...
	case DEC:
		...
		for (int i = 0; i < BLOCK_SIZE / 4; i++) {
			for (int j = 0; j < BLOCK_SIZE / 4; j++) {
				for (int k = 0; k < BLOCK_SIZE / 4; k++) {
					temp[i + 4 * j] ^= rprocession[4 * i + k] * block[4 * j + k];
				}
				block[i + 4 * j] = temp[i + 4 * j];
			}
		}
...
void AES128(BYTE* input, BYTE* result, BYTE* key, int mode) {
	if (mode == ENC) {
		...
		for (int i = 1; i < 10; i++) {
			memcpy(result, subBytes(input, mode), sizeof(BYTE) * 16);
			memcpy(result, shiftRows(input, mode), sizeof(BYTE) * 16);
			
			memcpy(result, mixColumns(input, mode), sizeof(BYTE) * 16);
			memcpy(result, addRoundKey(input, roundkey + 16 * i), sizeof(BYTE) * 16);
		}

		memcpy(result, subBytes(input, mode), sizeof(BYTE) * 16);
		memcpy(result, shiftRows(input, mode), sizeof(BYTE) * 16);
		memcpy(result, addRoundKey(input, roundkey + 16 * 10), sizeof(BYTE) * 16);
		...
		memcpy(result, subBytes(input, mode), sizeof(BYTE) * 16);
		memcpy(result, shiftRows(input, mode), sizeof(BYTE) * 16);
		memcpy(result, addRoundKey(input, roundkey + 16), sizeof(BYTE) * 16);
	}
	else {
		fprintf(stderr, "Invalid mode!\n");
		exit(1);
